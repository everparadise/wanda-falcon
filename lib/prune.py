...
def check_sparsity(model):
    ...
    for i in range(len(layers)):
        ...
        sub_params = 0
        for name in subset:
            print("check_sparsity subset name:",name)
...
def prepare_calibration_input(model, dataloader, device):
    ...
    return inps, outs, attention_mask, alibi
...
def prune_wanda(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    with torch.no_grad():
        inps, outs, attention_mask, alibi = prepare_calibration_input(model, dataloader, device)
